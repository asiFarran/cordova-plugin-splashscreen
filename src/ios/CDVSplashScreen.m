...
- (CGRect)rotateImageBounds:(CGRect)src
{
    CGFloat temp = src.size.width;
    src.size.width = src.size.height;
    src.size.height = temp;
    return src;
}
...
- (void)updateBounds
{
    ...
    CGFloat statusFrameAdjust = 0;
    
    // There's a special case when the image is the size of the screen.
    //if (CGSizeEqualToSize(screenSize, imgBounds.size)) {
    CGRect statusFrame = [self.viewController.view convertRect:[UIApplication sharedApplication].statusBarFrame fromView:nil];
    
    statusFrameAdjust = MIN(statusFrame.size.height, statusFrame.size.width);
    
    
    if (!(IsAtLeastiOSVersion(@"7.0"))) {
        imgBounds.origin.y -= statusFrameAdjust;
    ...
    else
    {
        ...
        switch (orientation) {
            ...
            case UIInterfaceOrientationLandscapeLeft:
                ...
                imgBounds = [self rotateImageBounds:imgBounds];
                break;
